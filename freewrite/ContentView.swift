...
struct ContentView: View {
    ...
    var placeholderOffset: CGFloat {
        return 0  // Remove the offset since we want it to align with the text
    }
                ...
                HStack(spacing: 0) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        ZStack {
                            ...
                            TextEditor(text: $text)
                                ...
                                .frame(
                                    maxWidth: showingAIChat ? 
                                        geometry.size.width - chatWidth - (showingSidebar ? 201 : 1) : 
                                        min(800, geometry.size.width - (showingSidebar ? 201 : 0))
                                )
                                .padding(.horizontal, showingAIChat ? 
                                    (showingSidebar ? 0 : 40) : 
                                    max(0, (geometry.size.width - (showingSidebar ? 201 : 0) - min(800, geometry.size.width - (showingSidebar ? 201 : 0))) / 2)
                                )
                                ...
                                HStack(spacing: 8) {
                                    ...
                                    Button("Chat") {
                                        if text.trimmingCharacters(in: .whitespacesAndNewlines).hasPrefix("hi. my name is farza.") {
                                            showingChatMenu = true // Keep the popover for guide text case
                                        } else {
                                            withAnimation(.easeInOut(duration: 0.2)) {
                    ...
                    if showingAIChat {
                        ...
                        VStack(spacing: 0) {
                            ...
                            AIChatView(text: $text)
                        }
                        .frame(width: chatWidth)
                        .background(Color(colorScheme == .light ? .white : .black))
                    }
                }
                .animation(.easeInOut(duration: 0.2), value: showingAIChat)
            }
        }
